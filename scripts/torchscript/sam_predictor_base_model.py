...
class TupleOutputSamPredictorBaseModel(nn.Module):
  ...
  def forward(self, image: torch.Tensor):
    ...
    # Remove batch dimensions from model outputs
...
class SamPredictorBaseModel(nn.Module):
  ...
  def __init__(self, model: Sam) -> None:
    super().__init__()
    self.sam = TupleOutputSamPredictorBaseModel(model)
  ...
  def forward(self, image: torch.Tensor):
    ...
    """
    outputs = self.sam(image)
    return {
        "masks": outputs[0],
        "iou_predictions": outputs[1],
        "low_res_logits": outputs[2],
    }
